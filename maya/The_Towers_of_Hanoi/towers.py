...
def show():
	'''
	Shows images plane, waits 2 seconds, then closes plane
	No arguments
	'''
	img.show()				#Shows imagae
	time.sleep(2)				#Waits 2 seconds
	commands.getoutput("killall eog")	#Closes window (command given to me by Ari)
	

def autoHanoi(numdisks, stacks, pole1, pole3, pole2):
	'''
	...
	numdisks	:	number of disks input by user
	stacks		:	the list 'stacks' - contains locations of disks on poles
	pole1		:	the left (source) pole
	pole3		:	the right (auxiliary) pole
	pole2		:	the middle (distination) peg
	'''
	if numdisks == 1:						#If numdisks is 1
		#print pole1, pole2, pole3				#Shows the current values of the poles
		pop = stacks[pole1].pop(0)				#Remove the top disk from the pole1 sub-list
		stacks[pole2].insert(0, pop)				#Add it to the beginning of pole2 sub-list
		background()						#Draws the background
		print "Stacks = ",stacks				#Shows the current setup of stacks
		drawDisks(numdisks)					#Draws the disks onto the poles
		show()							#Shows images, waits 2, closes
	else:								#Otherwise, if numdisks =/= 1
		autoHanoi(numdisks-1, stacks, pole1, pole2, pole3)	#Run the procedure again, taking away 1 from numdisks and making pole2 auxiliary and pole3 destination
		#print pole1, pole2, pole3
		pop = stacks[pole1].pop(0)				
		stacks[pole2].insert(0, pop)
		background()
		print "Stacks = ", stacks
		drawDisks(numdisks)
		show()
		autoHanoi(numdisks-1, stacks, pole3, pole1, pole2)	#Run the procedure again, taking away 1 from numdisks and making pole3 source, pole1 auxiliary and pole2 destination
...
progRunning = 1
while progRunning == 1:
	mode = str(raw_input("Would you like to PLAY the game, or WATCH it be completed for you? (Enter PLAY/WATCH) "))
	print " "
	
	if mode != "PLAY" and mode != "WATCH":
		if idiot < 5:
			...
			print "JUST TYPE IN 'WATCH' OR 'PLAY' YOU SMEG HEAD!"
			print " "
			idiot = 0
			progRunning = 1
	...
	if mode == "PLAY":
		...
		#print " "
		#print "Winning condition: ",WIN	#Shows the 'winning condition' for the game setup, i.e. all disks on middle peg/Commented out - only needed for construction use...
		...
		while (stacks != WIN):					#While the stacks are NOT in the 'winning condition'
			...
			if (move == "3 to 2"):
				if stacks[2] == []:
					...
					print "Move count = ",moveCount
					print " "
			if (move != "1 to 2" and not "1 to 3" and not "2 to 1" and not "2 to 3" and not "3 to 1" and not "3 to 2"):	#Basically, anything other than a valid move
				print " "
				print "*~*~ Invalid move! D:< ~*~*"
				print " "
		...
		if again == 'n':			#If user replies 'n' (no)...
			progRunning = 0			#modeBool bool changed to False, while loop ends...
		if again == 'y':
			programRunning = 1

	if mode == "WATCH":
		print "You chose to watch the game being completed for you."
		print " "
		comply = 0
		while (comply == 0):
			numdisks = raw_input("How many disks would you like to use? ")	#Asks user for a keyboard input for number of disks used in game
		...
		autoHanoi(numdisks, stacks, pole1=0, pole2=1, pole3=2)			#Call autoHanoi() procedure
		print " "
		print "Finished! It's easy when you know how! ;)"			#When the procedure has finished running, i.e. stacks = [[], [5,4,3,2,1], []]
		print " "
		print "It took", (2**numdisks)-1,"moves to complete this game. This is the minumum number of moves for",numdisks,"disks."
		print " "
		again = raw_input("Play again? (y/n) ")					#User asked whether to play again... (yes or no)
		if again == 'n':							#If user replies 'n' (no)...
